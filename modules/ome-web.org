...
programming techniques such as variables. Thus some nerds create some
programming languages which extend CSS with programming language features, and
"compiles" to css when it is necessary. [[http://www.sass-lang.org/][SASS]] is one of them.
#+BEGIN_SRC emacs-lisp
...
* helm-css-scss
  :PROPERTIES:
  :CUSTOM_ID: helm-css-scss
  :END:
  helm-css-scss

#+BEGIN_SRC emacs-lisp

(defun ome-helm-css-scss-setup ()
  (define-key scss-mode-map (kbd "C-c C-z") 'helm-css-scss)

)

(ome-install 'helm-css-scss)
